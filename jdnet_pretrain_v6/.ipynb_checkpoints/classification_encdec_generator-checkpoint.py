...
class ClassificationGenerator(object):
    ...
    def _generate(
        ...
        **kwargs
        ...
        decoder_outs = model.forward_decoder(src_tokens, encoder_outs[0])
        classification_out = model.forward_classifier(decoder_output=decoder_outs[0][0], src_lengths=src_lengths)
...
class EnsembleModel(torch.nn.Module):
    ...
    #junwei
    @torch.no_grad()
    def forward_decoder(self, src_tokens, encoder_output):
        if not self.has_decoder():
            return None
#         return [model.decoder(src_tokens, encoder_out=encoder_output, ALL=True) for model in self.models]
        return [model.decoder(src_tokens, encoder_out=encoder_output) for model in self.models]
    #junwei
    
    #junwei
    @torch.no_grad()
    def forward_classifier(self, decoder_output, src_lengths):
        if not self.has_classifier():
            return None
        return [model.classifier(decoder_out=decoder_output, lengths=src_lengths) for model in self.models]
    #junwei
    
    @torch.no_grad()
    def forward_decoder_back(self, tokens, encoder_outs, temperature=1.):
