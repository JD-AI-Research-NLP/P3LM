...
def makeOrder_ngram(num_skip, tensor, sequence_order, l2rseq_max, eye_matrix_max, padding_idx=-1):
    ...
    for n_skip in range(num_skip):
        ...
        for b in range(bsz):
            ...
            #get ngram indexes
            seq_expand_index = torch.tril(seq_column_expand, -n_skip -1)            
            ...
            end_see_seq = seq.unsqueeze(0).clone()
            if n_skip > 0:
                end_see_seq[0, -n_skip:] = 0
            ranked_index = torch.cat([ranked_index[1:], end_see_seq], 0)
...
def makeAll(tensor, sequence_order, l2rseq_max, padding_idx=-1):
    #sequence_order #[batch_size * max_length]
    bsz, max_length = sequence_order.size()
    masks = []
    seqs = []
    for b in range(bsz):        
        mask = tensor.new_zeros(max_length, max_length)
        masks.append(mask)
#     pdb.set_trace()
    return masks, seqs

def makeAll_ngram(num_skip, tensor, sequence_order, l2rseq_max, eye_matrix_max, padding_idx=-1):
    #sequence_order #[batch_size * max_length]
    bsz, max_length = sequence_order.size()
    ngram_masks = []
    ngram_seqs = []
    ...
    for n_skip in range(num_skip):
        ...
        for b in range(bsz):
            ...
            mask = utils.fill_with_neg_inf(tensor.new(max_length, max_length))
            mask1 = tensor.new_zeros(max_length, max_length)
            eye_matrix = eye_matrix_max[:max_length, :max_length]
            query = torch.where(eye_matrix==tensor.new_zeros(max_length, max_length), mask, seq_expand_value)
...
#         return attn, attn_weights_ngram
        return attn, None
