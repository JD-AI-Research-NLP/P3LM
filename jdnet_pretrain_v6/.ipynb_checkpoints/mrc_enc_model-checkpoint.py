...
@register_model('jdnet_mrc_enc')
class JDNetMRCEncModel(FairseqEncoderDecoderModel):
    """
    Args:
...
#         mlmDecoder = MLMDecoder(args, tgt_dict, mlmDecoder_embed_tokens)
        classifier = Classifier(args, tgt_dict)
        
        model = JDNetMRCEncModel(args, encoder, classifier)
...
#                 )            
        ...
        return JDNetMRCEncModel(args, encoder, classifier)
...
#     def __init__(self, args):
    def __init__(self, args, dictionary):
        super().__init__(dictionary)
#         self.register_buffer('version', torch.Tensor([3]))
        self.spanOutputs = nn.Linear(args.encoder_embed_dim, 2)
#         self.answerableOutputs = nn.Linear(args.encoder_embed_dim, 2)
        self.apply(init_bert_params)

    def forward(self, encoder_out=None, **kwargs):
        x = encoder_out['encoder_out'].transpose(0, 1)
        logits = self.spanOutputs(x)
        start_logits, end_logits = logits.split(1, dim=-1)
        encoder_out['start_end_logits'] = (start_logits , end_logits)
        
#         cls_representation = x[:, 0]
#         answerable_logit = self.answerableOutputs(cls_representation)
#         encoder_out['answerable_logit'] = answerable_logit
...
@register_model_architecture('jdnet_mrc_enc', 'jdnet_mrc_enc')
...
@register_model_architecture('jdnet_mrc_enc', 'jdnet_mrc_enc_base')
...
@register_model_architecture('jdnet_mrc_enc', 'jdnet_mrc_enc_middle')
...
@register_model_architecture('jdnet_mrc_enc', 'jdnet_mrc_enc_large')
