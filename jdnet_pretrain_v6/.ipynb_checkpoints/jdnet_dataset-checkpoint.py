...
from fairseq.data import data_utils, LanguagePairDataset, FairseqDataset

class MLMLanguagePairDataset(LanguagePairDataset):
    ...
    def __getitem__(self, index):
        ...
        pkgs['target'] = torch.zeros_like(src_item) + self.src_dict.pad_index
        pkgs['target'][masked_pos] = src_item[masked_pos].clone()
    ...
    def collate(self, samples, pad_idx, eos_idx, left_pad_source=True, left_pad_target=False):
        ...
        target = merge([s['target'] for s in samples], left_pad=left_pad_source)
...
class JDNetLanguageTripletDataset(LanguagePairDataset):
    """ Wrapper for masked language datasets 
        (support monolingual and bilingual)
        ...
        [x1, x2, x3, x4, x5] 
                 ||
                 VV
        [x1,  _,  _, x4, x5] => [_, x2, x3, _, _] => [x2, x3]
    ...
    def __getitem__(self, index):
        ...
        #===================Junwei ===================
        pkgs['target_mlm'] = torch.zeros_like(src_item) + self.src_dict.pad_index
        pkgs['target_mlm'][masked_pos] = src_item[masked_pos].clone()
        #===================Junwei ===================
        pkgs['prev_output_tokens'] = src_item[masked_pos - 1].clone()
        pkgs['positions'] = torch.LongTensor(masked_pos) + self.src_dict.pad_index
        src_item[masked_pos] = self.replace(src_item[masked_pos])
    ...
    def collate(self, samples, pad_idx, eos_idx, left_pad_source=True, left_pad_target=False):
        ...
        target_mlm = merge([s['target_mlm'] for s in samples], left_pad=left_pad_source)
...
class JDNetMRCDataset(FairseqDataset):
    """ Wrapper for masked language datasets 
        (support monolingual and bilingual)
    ...
    def __getitem__(self, index):
        ...
        positions = np.arange(0, len(self.src[index]))
        inputfeature = self.features[index]
        

        #===================Junwei ===================
        pkgs['start_position'] = inputfeature.start_position
        pkgs['end_position'] = inputfeature.end_position
        #===================Junwei ===================
        pkgs['positions'] = torch.LongTensor(positions) + self.src_dict.pad_index #yes
        pkgs['source'] = src_item #yes
...
#         target = merge([s['target'] for s in samples], left_pad=left_pad_target)
#         ++++++++++++++++++++++++++++++++++++
        start_positions = [s['start_position'] for s in samples]# To-Do
        end_positions = [s['start_position'] for s in samples]# To-Do
#         ++++++++++++++++++++++++++++++++++++
