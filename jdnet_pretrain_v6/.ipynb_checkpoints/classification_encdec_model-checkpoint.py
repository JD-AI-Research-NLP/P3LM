...
from .ngram_multihead_attention import NgramMultiheadAttention, ngram_attention_bias, makeOrder, makeOrder_ngram, makeAll, makeAll_ngram
import pdb
...
@register_model('jdnet_encdec_classification')
class JDNetEncDecClassificationModel(FairseqEncoderDecoderModel):
    """
    Args:
...
#         self.train_mlm = args.train_mlm
    ...
    def add_args(parser):
        ...
        parser.add_argument('--train-mlm', action='store_true',
                            help='pretrain mlm')
        parser.add_argument('--class-number', type=int, metavar='2',
                            help='class_number')
...
#         mlmDecoder = MLMDecoder(args, tgt_dict, mlmDecoder_embed_tokens)
        classifier = DecClassifier(args, tgt_dict)
        
        model = JDNetEncDecClassificationModel(args, encoder, decoder, classifier)
...
#             ----------Junwei
            for position_name, target_position_length in [('decoder.embed_positions.weight', model.decoder.embed_positions.weight.size(0))]:
                if states[position_name].size(0) < target_position_length:
                    _index = torch.arange(states[position_name].size(1))
                    expend_position_states = states[position_name].clone()
                    while states[position_name].size(0) < target_position_length:
...
#             ----------Junwei
        ...
        return JDNetEncDecClassificationModel(args, encoder, decoder, classifier)
...
#         pdb.set_trace()
        ...
        elif 'EncClassificationOnly' in kwargs:
            encoder_out = kwargs['encode_out']
            classification_out = self.classifier(encoder_out=encoder_out, **kwargs)
            return classification_out
        elif 'DecodeOnly' in kwargs:
            encoder_out = kwargs['encode_out']
            decoder_out = self.decoder(src_tokens, encoder_out=encoder_out, **kwargs)
            return decoder_out
        elif 'DecClassificationOnly' in kwargs:
            decoder_out = kwargs['decode_out']
#             pdb.set_trace()
            classification_out = self.classifier(decoder_out=decoder_out, lengths=src_lengths, **kwargs)
            return classification_out
        else:
            encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, **kwargs)
            decoder_out = self.decoder(prev_output_tokens, encoder_out=encoder_out, **kwargs)
...
#         pdb.set_trace()
        ...
        if 'ALL' in unused:
            self_attn_mask_plm = self.buffered_future_mask_all(x, sequence_order, l2rseq_max, self.padding_idx - 1) if incremental_state is None else None
            plm_ngram_mask_matrix = self.buffered_future_mask_ngram_all(x, sequence_order,  l2rseq_max, eye_matrix_max, self.padding_idx - 1) if incremental_state is None else None
        else:
...
#         pdb.set_trace()
    ...
    def buffered_future_mask_all(self, tensor, order, l2rseq_max, padding_idx):
        dim = tensor.size(0)
        self._future_mask_all, seqs = makeAll(tensor, order, l2rseq_max, padding_idx)
        self._future_mask_all = torch.cat([x.unsqueeze(0) for x in self._future_mask_all], 0)
        return self._future_mask_all[:,:dim, :dim]
    
    def buffered_future_mask_ngram_all(self, tensor, order, l2rseq_max, eye_matrix_max, padding_idx): #order: batch * T
        dim = tensor.size(0)
        self._ngram_future_mask_all, ngram_seqs = makeAll_ngram(self.ngram, tensor, order, l2rseq_max, eye_matrix_max, padding_idx)
        return self._ngram_future_mask_all

class Classifier(FairseqDecoder):
#     def __init__(self, args):
    def __init__(self, args, dictionary):
        super().__init__(dictionary)
        self.register_buffer('version', torch.Tensor([3]))
        self.outputs = nn.Linear(args.encoder_embed_dim, args.class_number)
        self.apply(init_bert_params)

    def forward(self, encoder_out=None, **kwargs):
        x = encoder_out['encoder_out'].transpose(0, 1)
        first_token_tensor = x[:, 0:1, :].squeeze(1)
        logits = self.outputs(first_token_tensor)
        encoder_out['classify_logits'] = logits
        return encoder_out

class DecClassifier(FairseqDecoder):
#     def __init__(self, args):
    def __init__(self, args, dictionary):
        super().__init__(dictionary)
        self.register_buffer('version', torch.Tensor([3]))
    ...
    def forward(self, decoder_out=None, lengths=None, **kwargs):
        x = decoder_out #.transpose(0, 1) #[B, T, D] => [T, B, D]

        first_token_tensor = x[:, 0:1, :].squeeze(1)   
        hidden_dim = x.size(-1)
        indexes = (lengths-3).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, hidden_dim)
        last_token_tensor = torch.gather(x, 1, indexes).squeeze(1)
        
#         logits = self.outputs(first_token_tensor)
        logits = self.outputs(last_token_tensor)

        classify_out = {}
        classify_out['classify_logits'] = logits
        return classify_out
...
@register_model_architecture('jdnet_encdec_classification', 'jdnet_encdec_classification')
...
@register_model_architecture('jdnet_encdec_classification', 'jdnet_encdec_classification_base')
...
@register_model_architecture('jdnet_encdec_classification', 'jdnet_encdec_classification_middle')
...
@register_model_architecture('jdnet_encdec_classification', 'jdnet_encdec_classification_large')
